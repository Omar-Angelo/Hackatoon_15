...
///Detalle
const detallepadre = document.querySelector('#detallepadre')
const detalle = document.querySelectorAll('#detalle')
const cerrardeta = document.querySelector('.ceri')

detalle.forEach(deta=>{
  deta.addEventListener('click',MosDeta)
})
function MosDeta(){
  detallepadre.classList.add('detalleactive')
}

cerrardeta.addEventListener('click',Cerrardeta)
function Cerrardeta(){
  detallepadre.classList.remove('detalleactive')
}
...
elimiar1.forEach(elima=>{
  elima.addEventListener('click',Eliminado)
})
function Eliminado(e){
  let confirmacion =confirm('Seguro que deseas eliminar')
  if(confirmacion === true ) { e.target.parentElement.parentElement.parentElement.remove()
}
